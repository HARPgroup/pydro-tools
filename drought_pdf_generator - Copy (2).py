...
def main():
    ...
    # pdf.cell(40, 10, 'Daily Drought Status Report')
    # pdf.cell(40, 20, today)
    # pdf.output('test1.pdf', 'F')
    pdf.cell(60)
    pdf.cell(75, 10, 'test text', 0, 2, 'C')
    pdf.cell(90, 10, '', 0, 2, 'C')
    pdf.cell(-55)
    columnNameList = list(sw_status_df_all.columns)
    for header in columnNameList[:-1]:
        pdf.cell(35, 10, header, 1, 0, 'C')
    pdf.cell(35, 10, columnNameList[-1], 1, 2, 'C')
    pdf.cell(-140)
    pdf.set_font('Arial', 'B', 16)
    for row in range(0, len(sw_status_df_all)):
        for col_num, col_name in enumerate(columnNameList):
            if col_num != len(columnNameList) - 1:
                pdf.cell(35, 10, str(sw_status_df_all['%s' % (col_name)].iloc[row]), 1, 0, 'C')
            else:
                pdf.cell(35, 10, str(sw_status_df_all['%s' % (col_name)].iloc[row]), 1, 2, 'C')
                pdf.cell(-140)          
    pdf.cell(35, 10, "", 0, 2)
    pdf.cell(20)
